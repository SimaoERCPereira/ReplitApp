...
# For production, use DATABASE_URL environment variable (provided by Railway)
...
app.config['SQLALCHEMY_DATABASE_URI'] = database_url or 'mssql+pyodbc://SIMÃO-ASUS-ROG\LOCALSERVER/ThePeoplesPitch?driver=ODBC+Driver+17+for+SQL+Server&trusted_connection=yes'
...
TEAM_CREST_URL_TEMPLATE = 'https://resources.premierleague.com/premierleague/badges/rb/t{team_code}.svg'
...
def parse_fpl_datetime(datetime_str):
    ...
    try:
        ...
        print(f"Error: Could not parse datetime string '{datetime_str}' with fromisoformat.")
        return None
...
def fetch_and_update_fpl_teams_standings():
    try:
        ...
        # Diagnostic print: Show top-level keys from API response
        ...
        # Diagnostic print: Show how many teams were received and data for the first team
        ...
        for team_data in teams_data:
            ...
            if not team:
                team = Team.query.filter(db.func.lower(Team.name) == db.func.lower(team_data['name'])).first()
                if not team:
                    team = Team()
                    db.session.add(team)
                    new_teams += 1
            ...
            if isinstance(raw_form, str): # If API provides a ready-to-use string form
                ...
                try:
                    ...
                    print(f"DEBUG: Could not convert form list to string for team {team_data.get('name')}: {raw_form}")
                    team.form = None # Fallback if conversion fails
            else: # If form is None, or not a string/list we can easily handle
                team.form = None
            
            # Enhanced diagnostic print for the first team being processed
            if teams_data and team_data['id'] == teams_data[0]['id']:
                 print(f"DEBUG: --- Processing Team from API ---")
                 print(f"DEBUG: API Raw Data: id={team_data.get('id')}, name='{team_data.get('name')}', short_name='{team_data.get('short_name')}', code={team_data.get('code')}")
                 print(f"DEBUG: API Stats: played={team_data.get('played')}, win={team_data.get('win')}, draw={team_data.get('draw')}, loss={team_data.get('loss')}, points={team_data.get('points')}, position={team_data.get('position')}")
                 print(f"DEBUG: API Form Raw: {team_data.get('form')}")
                 print(f"DEBUG: --- Assigning to DB Model ({team.name}) ---")
                 print(f"DEBUG: DB Model Values: fpl_id={team.fpl_id}, name='{team.name}', short_name='{team.short_name}', fpl_team_code={team.fpl_team_code}")
                 print(f"DEBUG: DB Model Stats: played={team.played}, win={team.win}, draw={team.draw}, loss={team.loss}, points={team.points}, position={team.position}")
...
def fetch_and_update_fpl_fixtures():
    try:
        ...
        for fixture_data in fixtures_data:
            ...
            home_team_obj = Team.query.filter_by(fpl_id=fixture_data.get('team_h')).first()
            away_team_obj = Team.query.filter_by(fpl_id=fixture_data.get('team_a')).first()

            if not home_team_obj or not away_team_obj:
                print(f"Warning: Could not find one or both teams for fixture ID {fixture_data['id']}. Home FPL ID: {fixture_data.get('team_h')}, Away FPL ID: {fixture_data.get('team_a')}. Skipping this fixture.")
                continue
            ...
            match.gameweek = fixture_data.get('event') # Populate gameweek from FPL API 'event' field

            if fixture_data.get('started') and not fixture_data.get('finished_provisional') and not fixture_data.get('finished'):
                match.status = 'Live'
...
def home():
    ...
    gameweeks_query = db.session.query(Match.gameweek).filter(Match.gameweek.isnot(None)).distinct().order_by(Match.gameweek.asc()).all()
    ...
    if current_display_gameweek is None and all_gameweeks:
        ...
        first_relevant_gameweek = db.session.query(Match.gameweek)\
...
def update_fpl_data_route():
    ...
    flash(fixture_update_msg, 'success' if 'Error' not in fixture_update_msg else 'danger')
...
def match_discussion(match_id):
    ...
    if request.method == 'POST':
        if not current_user.is_authenticated:
            ...
            return redirect(url_for('login', next=url_for('match_discussion', match_id=match_id)))
        
        comment_text = request.form.get('comment')
        if comment_text and len(comment_text) <= 280:
            new_comment = Comment(text=comment_text, user_id=current_user.id, match_id=match.id)
    ...
    return render_template('match_discussion.html', match=match, comments=sorted(comments, key=lambda c: c.timestamp), title=f"{match.home_team.name} vs {match.away_team.name} Discussion")
...
def team_page(team_name):
    ...
    if team:
        ...
        team_details = {
            ...
            "form": team.form,
            "news": [
                {"title": "New Player Signed!", "summary": "The team announced the signing of a new striker today.", "date": "May 12"}, 
                {"title": "Captain Injured", "summary": "The captain will be out for 3 weeks due to a knee injury.", "date": "May 11"},
                {"title": "Next Match Preview", "summary": "An in-depth look at the upcoming clash.", "date": "May 10"}
            ], 
            "fans_count": 1250,
    ...
    return render_template('team_page.html', team=team_details, title=f"{team_name} Team Page")
